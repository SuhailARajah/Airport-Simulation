...
class Simulator {
private:
	int total_time = 10080; // total time simulation will be ran for one week
...
public:
	...
	void enter_data(){
		...
		std::cout << "We are a small hospital with a big heart\n";
		...
		//number of nurses must be between 1-2 
		int num_nurses = read_int("Please enter number of nurses working: ", 1, 2);
		emergency->setNumNurse(num_nurses);

		//number of doctors must be between 1-2
		int num_doctors = read_int("Please enter number of doctors working: ", 1, 2);
	...
	void post_simulation(){
		...
		int select = 0;
		while (select != 3){
			std::cout << "Please choose one of the follwoing:\n";
